...
# Função de validação para slugs de espécies e categorias
def validate_and_sanitize_slug(name: str) -> str:
    """
    Valida e gera um slug seguro a partir de um nome.

    Args:
        name: Nome a partir do qual o slug será gerado

    Returns:
        Slug sanitizado e seguro
    """
    from slugify import slugify

    # Primeiro sanitiza o nome
    sanitized_name = InputValidator.sanitize_name(name)

    # Gera o slug com a biblioteca slugify
    slug = slugify(sanitized_name)

    # Verifica se o slug é válido
    is_valid, _ = InputValidator.validate_slug(slug)

    if not is_valid or not slug:
        # Em caso de erro, gera um slug genérico com timestamp
        import time
        slug = f"item-{int(time.time())}"
