...
class UserService:
    ...
    def login_user(self, user_input: UserCreate) -> TokenData:
        ...
        try:
            ...
            if not user or not user.is_active:
                logger.warning(f"Tentativa de login com usuário inexistente ou inativo: {user_input.email}")
    ...
    def delete_user_permanently(self, user_id: UUID) -> dict:
        ...
        Raises:
            ResourceNotFoundException: Se o usuário não for encontrado
            DatabaseOperationException: Se houver erro no processo ou se
                                       o usuário tiver pets associados
        """
        try:
            user = self._get_user_by_id(user_id)
            ...
            if pets_count > 0:
                logger.warning(f"Tentativa de excluir usuário com pets vinculados: {user.email} ({pets_count} pets)")
                raise DatabaseOperationException(
